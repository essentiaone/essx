...
const int TIER_MERCURY_BASE_COLLATERAL   = 100000;
const int TIER_MARS_BASE_COLLATERAL   = 300000;

static size_t GetHashRoundsForTierMasternodes(CMasternode::Tier tier)
{
    switch(tier)
    {
    case CMasternode::MASTERNODE_TIER_MERCURY:   return 20;
    case CMasternode::MASTERNODE_TIER_MARS:      return 63;
...
CAmount CMasternode::GetTierCollateralAmount(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_MERCURY:   return TIER_MERCURY_BASE_COLLATERAL * COIN;
    case MASTERNODE_TIER_MARS:   return TIER_MARS_BASE_COLLATERAL * COIN;
...
{
    if(TIER_MERCURY_BASE_COLLATERAL * COIN == nCollateral) {
        return MASTERNODE_TIER_MERCURY;
    }
    else if(TIER_MARS_BASE_COLLATERAL * COIN == nCollateral) {
        return MASTERNODE_TIER_MARS;
    }
...
bool CMasternode::IsTierValid(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_MERCURY:
    case MASTERNODE_TIER_MARS: return true;
    case MASTERNODE_TIER_INVALID: break;
    }
...
std::string CMasternode::TierToString(CMasternode::Tier tier)
{
    switch(tier)
    {
    case MASTERNODE_TIER_MERCURY: return "MERCURY";
    case MASTERNODE_TIER_MARS: return "MARS";
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 31488) return false;
    } else if (addr.GetPort() == 31589)
        return false;
