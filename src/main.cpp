...
#if defined(NDEBUG)
#error "Essentia cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zEss spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zEss spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zEss spend with serial %s from tx %s is not in valid range\n",
...
CBlockRewards GetBlockSubsidity(int nHeight)
{
    ...
    if(nHeight <= Params().LAST_POW_BLOCK()) {
        return CBlockRewards(nSubsidy, 0, 0, 0);
    }

    auto helper = [nSubsidy](int nStakePercentage, int nMasternodePercentage, int nTreasuryPercentage, int nProposalsPercentage) {
        auto helper = [nSubsidy](int percentage) {
            return (nSubsidy * percentage) / 100;
        };

        return CBlockRewards(helper(nStakePercentage), helper(nMasternodePercentage), helper(nTreasuryPercentage), helper(nProposalsPercentage));
    };
    ...
    if(sporkManager.IsSporkActive(SPORK_13_BLOCK_PAYMENTS)) {
        ...
        if(activeSpork.IsValid()) {
            ...
            return helper(activeSpork.nStakeReward, activeSpork.nMasternodeReward,
                          activeSpork.nTreasuryReward, activeSpork.nProposalsReward);
        }
    }

    return helper(38, 45, 17, 0);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Essentia
...
{
    RenameThread("ess-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZESSMinted()
{
...
void RecalculateZESSSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zESS supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zEssSpent: %s\n",
    ...
    CBlockRewards nPoWExpectedMint(nBaseExpectedMint.nStakeReward + nFees, 0, 0, 0);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    */
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zEss spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zEss serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zEss mints and %d zEss spends\n", __func__, nMints, nSpends);
...
{
    return strprintf("BlockRewards(nStakeReward=%s, nMasternodeReward=%s, nTreasuryReward=%s, nProposalsReward=%s)",
                       FormatMoney(nStakeReward), FormatMoney(nMasternodeReward), FormatMoney(nTreasuryReward), FormatMoney(nProposalsReward));
}

CAmount CBlockRewards::TotalReward() const
{
    return nStakeReward + nMasternodeReward + nTreasuryReward + nProposalsReward;
}