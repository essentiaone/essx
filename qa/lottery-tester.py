...
def main(argv):
    ...
    parser.add_argument('--prefix', dest='prefix', help='path to folder where ess-cli is located', default='.')
    ...
    executableName = prefix + '/ess-cli'
    print executableName

    if os.path.isfile(executableName) == False:
        print 'Cannot find ess-cli, consider using --prefix'
        sys.exit(0)

    try:
        json.loads(executeProcess([executableName, 'getinfo']))
    except ValueError:
        print 'Ess-cli is not running, or it is in wrong dir'
        sys.exit(0)
